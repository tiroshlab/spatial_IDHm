...
for(i in seq_along(sample_ls)) {
  m <- readRDS(paste("/exp_mats_GBM/", sample_ls[i], "counts.rds", sep = ""))
...
file_list <- list.files("/mp_assign_full_generalized_v2/")
gen_clusters <- as.character(unique(unlist(sapply(c(1:49), function(i){
  mp_assign <- readRDS(paste("/mp_assign_full_generalized_v2/", file_list[i], sep = ""))
  return(unique(mp_assign$spot_type_meta_new))
}))))
...
# load data
spots_positions <- read.csv(paste("github/data/", sname , "/outs/spatial/tissue_positions_list.csv", sep = ""), header = FALSE, stringsAsFactors = FALSE)
row.names(spots_positions) <- spots_positions$V1

spots_clusters <- readRDS(paste("/mp_assign_full_generalized_v2/", sname, ".rds", sep = ""))
spots_clusters <- na.omit(spots_clusters)
colnames(spots_clusters) <- c("barcodes", "spot_type")
row.names(spots_clusters)<- spots_clusters$barcodes  
...
sname <- "" #insert sample name 
file_list <- list.files("/mp_assign_full_generalized_v2/")
gen_clusters <- as.character(unique(unlist(sapply(c(1:50), function(i){
  mp_assign <- readRDS(paste("/mp_assign_full_generalized_v2/", file_list[i], sep = ""))
  return(unique(mp_assign$spot_type_meta_new))
}))))
...
# load data
spots_positions_orign <- read.csv(paste("github/data/", sname , "/outs/spatial/tissue_positions_list.csv", sep = ""), header = FALSE, stringsAsFactors = FALSE)
...
samp_list <- list.files("/mp_assign_full_generalized_v2/")

pairs <- combn(sort(gen_clusters),2)
pairs_names <- apply(pairs, 2, function(x){return(paste(x[1],x[2], sep = " "))})

all_proximity <- readRDS("results/region_comp/mp_assign_full_generalized_v2/idhmut_prox.rds")

names(all_proximity) <- sample_ls


all_proximity_rand_list <- list.files("/results/region_comp_rand_comb_gen/")

all_proximity_rand <- lapply(all_proximity_rand_list, function(prox){
  samp_prox <- readRDS(paste("/results/region_comp_rand_comb_gen/", prox, sep = ""))
  return(samp_prox)
})
names(all_proximity_rand) <- substr(all_proximity_rand_list,1,sapply(all_proximity_rand_list,function(s){nchar(s)-19}))
...
spots_numv1 <-sapply(samp_list, function(smp){
  samp_df <- readRDS(paste("/mp_assign_full_generalized_v2/", smp, sep = ""))
  return(nrow(samp_df))
})
...
proximity_bin <- lapply(c(1:50),function(i){
  r1 = all_proximity[[i]]
